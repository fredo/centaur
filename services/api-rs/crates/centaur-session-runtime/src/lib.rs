...
impl SandboxWorkloadMode {
    ...
    pub fn codex_app_server(
        ...
        Self::CodexAppServer {
            ...
            mounts: Vec::new(),
            // Matches the sandbox image's default CMD (services/sandbox/Dockerfile).
    ...
    /// Set the default harness: what warm sandboxes boot with. Per-session
    /// sandboxes always run the session's own harness.
    pub fn default_harness_type(mut self, harness_type: HarnessType) -> Self {
        match &mut self {
            Self::MockAppServer { .. } => {}
            Self::CodexAppServer { harness, .. } => *harness = harness_type,
        }
        self
    }
...
mod tests {
    ...
    fn warm_spec_uses_workload_default_harness() {
        let workload = SandboxWorkloadMode::codex_app_server("centaur-agent:latest", Vec::new())
            .default_harness_type(HarnessType::Codex);
