...
describe('forwardToSessionApi harness restart', () => {
  ...
  test('harness_switched response fires onSessionRestarted and prepends the preamble', async () => {
    ...
    const execute = requests.find(request => request.url.endsWith('/execute'))
    const line = JSON.parse((execute?.body as { input_lines: string[] }).input_lines[0]!)
    const [first, second] = line.message.content
    expect(first.type).toBe('text')
    expect(first.text).toContain('restarted on a different agent harness')
    expect(first.text).toContain('[test]: earlier question')
    expect(first.text).toContain('[assistant]: earlier answer')
    expect(first.text).not.toContain('continue with codex')
    expect(second).toEqual({ type: 'text', text: 'continue with codex' })
  })
  ...
  test('no restart leaves the execute line without a preamble', async () => {
    ...
    const execute = requests.find(request => request.url.endsWith('/execute'))
    const line = JSON.parse((execute?.body as { input_lines: string[] }).input_lines[0]!)
    expect(line.message.content).toEqual([{ type: 'text', text: 'plain message' }])
  })
})
